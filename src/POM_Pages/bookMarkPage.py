...
class BookMark():
    ...
    first_video_tab=(By.XPATH,("//android.widget.FrameLayout[@resource-id='com.byjus.thelearningapp:id/chapter_view_group' and @index = 1]"))
            ...
            try:
                ...
                if CommonMethods.wait_for_element_visible(browser, self.homescreen_corana_dialog, 6):
                   ...
                   # VideoPage.subject_rgb_lst = self.get_the_rgb_lst(browser, subject_rgb)
                    ...
                    self.verify_home_page(browser)
                   # VideoPage.subject_rgb_lst = self.get_the_rgb_lst(browser, subject_rgb)
                elif CommonMethods.wait_for_element_visible(browser, self.allow_btn_id, 3) or CommonMethods.wait_for_element_visible(browser, self.loginBtn_id, 3) :
    ...
    def subject_tap(self,browser,text):
        ...
        try:
            ...
            if check==True:
                ...
                browser.find_element_by_android_uiautomator('new UiScrollable(new UiSelector().scrollable(true)).setAsHorizontalList().scrollIntoView(new UiSelector().textContains("'+text+'"))')
                CommonMethods.elementClick(browser, sub_opt)
