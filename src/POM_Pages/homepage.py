...
class HomePage():
    country_Code_id = (By.ID, "com.byjus.thelearningapp:id/spnrCountry")
    phone_num_id = (By.ID, "com.byjus.thelearningapp:id/etPhoneNumber")
    ...
    loginBtn_id = (By.ID, "com.byjus.thelearningapp:id/btnLogin")
    OtpTxtBx_id = (By.ID, "com.byjus.thelearningapp:id/etOTP")
    covid19_pop_up=(By.ID,"com.byjus.thelearningapp:id/dialog_layout")
    
#     homescreen_corana_dialog_ok_btn = (By.ID,"com.byjus.thelearningapp:id/bt_primaryAction")
    none_of_the_above_id = (By.ID,"com.google.android.gms:id/cancel")
    ...
    covid19_secondary_ok_opt = (By.ID,"com.byjus.thelearningapp:id/tv_secondaryAction")
    covid19_primary_reserve_noe_opn = (By.ID,"com.byjus.thelearningapp:id/bt_primaryAction")
...
#         browser.find_element_by_xpath(self.btn_mathematics).click()
    ...
    def verify_home_page(self, browser):
        try:    
            if CommonMethods.isElementPresent( browser, self.back_button_id)==True:
                ...
                actual_mob = getdata(Login_Credentials,'login_details2', 'mobile_no')
                actual_mob_code=getdata(Login_Credentials,'login_details2', 'code')
            ...
            logging.info('Error in verify_home_page method in Homepage')
     
    def navigate_to_home_screen(self, browser):
        try:
            if CommonMethods.isElementPresent( browser, self.homescreen_corana_dialog) == True:
    #             if CommonMethods.wait_for_element_visible(browser, self.homescreen_corana_dialog, 3):
                CommonMethods.elementClick(browser, self.covid19_secondary_ok_opt)
    ...
    #                     self.verify_badge(browser)
                ...
                self.verify_home_page(browser)
                self.handle_covid19_popup_secondary_ok_opn(browser)
    #             elif CommonMethods.wait_for_element_visible(browser, self.loginBtn_id, 3) or CommonMethods.wait_for_element_visible(browser, self.allow_btn_id, 3)  :
    ...
    def verify_to_login_page_for_varient_c(self, browser):
            if CommonMethods.wait_for_element_visible(browser, self.allow_btn_id, 3):
                ...
                CommonMethods.elementClick(browser,self.country_Code)
                sleep(1)
                CommonMethods.scrollToElementAndClick(browser, getdata(Login_Credentials, 'login_details4_varientC', 'country_code'))
                CommonMethods.enterText(browser, getdata(Login_Credentials, 'login_details4_varientC', 'mobile_no'), self.phone_num)
                CommonMethods.wait_for_locator(browser, self.loginBtn_id, 15)
                CommonMethods.elementClick(browser, self.loginBtn_id)
                CommonMethods.wait_for_locator(browser, self.OtpTxtBx_id, 15)
                CommonMethods.enterText(browser, getdata(Login_Credentials, 'login_details4_varientC', 'OTP'), self.OtpTxtBx_id)
            elif CommonMethods.wait_for_element_visible(browser, self.loginBtn_id, 3):
                CommonMethods.wait_for_locator(browser,self.country_Code,15)
                CommonMethods.elementClick(browser,self.country_Code)
                sleep(2)
