...
class Bookmark_filter():
   ...
   # Bookmark_icon=(By.ID,"//com.byjus.thelearningapp:id/ivBookmark")
    ...
    def filter_tap(self,browser):
        try:
            
            if CommonMethods.wait_for_element_visible( browser, self.filter_txt, 3):
                CommonMethods.elementClick( browser, self.filter_txt)
            else:
                logging.info('Failed to click on filter ')
                pytest.fail("Failed to click on filter")
        except NoSuchElementException :
            CommonMethods.noSuchEleExcept(browser,featureFileName,'filter_tap')  
               
        except:
            CommonMethods.exception(browser, featureFileName, 'filter_tap')
            
    ''' filter_close_verify verifies filter close button in bookmarkscreen     '''
            
    def filter_close_verify(self,browser):
        try:
            if CommonMethods.wait_for_element_visible(browser, self.filter_close,5):
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'filter_close_verify')  
               
        except:
            CommonMethods.exception(browser, featureFileName, 'filter_close_verify')
            
    ''' verify_filter_txt verifies filter text in  bookmarkscreen     '''
    
    def verify_filter_txt(self,browser,text):
        try:
            check=CommonMethods.isElementPresent( browser, self.filter_innertxt)
             
            if check == True:
                
                act_txt=CommonMethods.getTextOfElement(browser,self.filter_innertxt)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'verify_filter_txt')    
        except:
            CommonMethods.exception(browser, featureFileName, 'verify_filter_txt')
            
    ''' showalltxt_btn verifies both text and button in the filter option    '''        
            
    def showalltxt_btn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.Showall_txt)
                         
            if check == True:
                logging.info("showall option is present")
                CommonMethods.isElementPresent( browser, self.Showall_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'showalltxt_btn')    
        except:
            CommonMethods.exception(browser, featureFileName, 'showalltxt_btn')
            
    ''' questiontxt_btn verifies both question  text and button in the filter option    ''' 
            
    def questiontxt_btn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.questions_txt)
                         
            if check == True:
                logging.info("questions option is present")
                CommonMethods.isElementPresent( browser, self.questions_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'questiontxt_btn')    
        except:
            CommonMethods.exception(browser, featureFileName, 'questiontxt_btn')
            
            
    ''' videos_txt_btn verifies both question  text and button in the filter option    '''           
    def videos_txt_btn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.questions_txt)
                         
            if check == True:
                logging.info("questions option is present")
                CommonMethods.isElementPresent( browser, self.videos_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'videos_txt_btn')    
        except:
            CommonMethods.exception(browser, featureFileName, 'videos_txt_btn')   
            
            
    ''' filter_closebtn verifies filter close btn and clicks if it is present     '''                    
    def filter_closebtn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.filter_close)
                         
            if check == True:
                
                CommonMethods.elementClick( browser,self.filter_close )
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'filter_closebtn')    
        except:
            CommonMethods.exception(browser, featureFileName, 'filter_closebtn')
            
            
    ''' filterscreen_verify verifies filter screen is present or not     '''                     
    def filterscreen_verify(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.filter_screen)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'filterscreen_verify')    
        except:
            CommonMethods.exception(browser, featureFileName, 'filterscreen_verify') 
    ...
    def bookmark_question(self,browser):
        try:
            self.navigate_to_library(browser, 'Mathematics')
            check=CommonMethods.isElementPresent(browser, self.Library_test)
            if check == True:
                
                CommonMethods.elementClick(browser,self.Library_test)
                CommonMethods.wait_for_element_visible( browser, self.take_test, 3)
                CommonMethods.elementClick(browser,self.take_test)
                CommonMethods.wait_for_element_visible( browser, self.start_test, 3)
#                 check_initial=CommonMethods.isElementPresent(browser,self.start_test)
#                 if check_initial== True:
                CommonMethods.elementClick(browser,self.start_test)
                sleep(5)
                    
                CommonMethods.elementClick(browser,self.bookmark_question)
                browser.back()
                CommonMethods.wait_for_element_visible( browser, self.Abort_btn, 3)
                CommonMethods.elementClick(browser,self.Abort_btn)
                sleep(2)
                #browser.back()
                #browser.back()
                CommonMethods.click_on_device_back_btn(browser)
...
#                 logging.info("failed to navigate to library list ")    
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'bookmark_question')
        
        except :
            CommonMethods.exception(browser,featureFileName,'bookmark_question')
    ...
    def bookmark_question_verify(self,browser):
        try:
            check=CommonMethods.isElementPresent(browser, self.bmrkquestion_verify)
            if check == True:
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'bookmark_question_verify')
        
        except :
            CommonMethods.exception(browser,featureFileName,'bookmark_question_verify')
    ...
    def showall_selected(self,browser):
        try:             
            high=CommonMethods.getAttributeOfElement( browser,'selected',self.showa11_btn ) 
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'showall_selected')
        
        except:
            CommonMethods.exception(browser, featureFileName, 'showall_selected' )  
            
             
    ''' tap_showall method will verify  and tap on showwll option from filter  '''
    def tap_showall(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.Showall_btn)
                         
            if check == True:

                CommonMethods.elementClick( browser, self.Showall_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'tap_showall')    
        except:
            CommonMethods.exception(browser, featureFileName, 'tap_showall')
            
            
    ''' tap_filter_video_btn method will verify  and tap on videos option from filter  '''            
    def tap_filter_video_btn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.videos_btn)
                         
            if check == True:

                CommonMethods.elementClick( browser, self.videos_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'tap_video')    
        except:
            CommonMethods.exception(browser, featureFileName, 'tap_video')
            
    ''' tap_filter_question_btn method will verify  and tap on question  option from filter  '''        
    def tap_filter_question_btn(self,browser):
        try:
            check=CommonMethods.isElementPresent( browser, self.questions_btn)
                         
            if check == True:

                CommonMethods.elementClick( browser, self.questions_btn)
            ...
            CommonMethods.noSuchEleExcept(browser,featureFileName,'tap_filter_question_btn')    
        except:
            CommonMethods.exception(browser, featureFileName, 'tap_filter_question_btn')
            
    ''' all_highlight_Verify method will verify  the highlight is present on the selected option  '''            
    def all_highlight_Verify(self,browser):        
        try:
            
            high=CommonMethods.getAttributeOfElement( browser, 'selected', self.all_txt)
            print("value of the high........"+high)
            if (high == "true"):
                logging.info("All is highlighted ")
                CommonMethods.isElementPresent(browser, self.bookmark_icon)
            else:
                logging.info('subject failed to highlight')
                pytest.fail("Failed due to Bookmark subject highlight")
         
        except NoSuchElementException :
            CommonMethods.noSuchEleExcept(browser,featureFileName,'all_highlight_Verify')
        
        except:
            CommonMethods.exception(browser, featureFileName, 'all_highlight_Verify')
            
            
    ''' Bookmark_video_removedverify method will verify  bookmarked video is removed in bookmarkscreen   '''            
    def Bookmark_video_removedverify(self,browser):
        try:
            check=CommonMethods.isElementPresent(browser, self.bmrkvideo_verify)
            if check == False:
