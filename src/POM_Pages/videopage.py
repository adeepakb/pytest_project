...
#     Journey Locators
    ...
    def verify_video_playing(self,driver):
        try:   
            if CommonMethods.wait_for_element_visible(driver, self.video_list_btn_tab, 3) or CommonMethods.wait_for_element_visible(driver, self.video_list_close_btn_tab, 3):
                pass  
            sleep(3)
...
#                     pytest.fail("Failed Due to Locator in Video Page")
    ...
    def wait_till_video_complete(self, driver, time):
        ...
        loading_count = 0
        try:
            while not CommonMethods.wait_for_element_visible(driver, self.video_auto_cancelBtn_id, 2) and loading_count < time+100:
                loading_count += 1
                sleep(0.2)
            return True
        except:
...
#                     pytest.fail("Failed due to Element not visible in Video Page")
    ...
    def verify_playback_speed_buttom_sheet(self, driver):
        try:
            ...
            self.verify_true_or_false(driver, check, 'verify_playback_speed_buttom_sheet', "play back speed bottom dialog")
...
#                     pytest.fail("Failed due to Element not visible in Video Page")
    ...
    def wait_video_to_complete(self, driver, wait_time):
        ...
        loading_count = 0
        try:
            while not CommonMethods.wait_for_element_visible(driver, self.video_auto_cancelBtn_id, 1) and loading_count < wait_time+100:
                loading_count += 1
                sleep(0.2)
        except:
    ...
    def wait_for_video_in_practice(self, driver):
        ...
        loading_count = 0
        try:
            while not CommonMethods.wait_for_element_visible(driver, self.practice_continue_btn, 2) and loading_count < 75:
                loading_count += 1
                ...
                elif CommonMethods.wait_for_element_visible(driver, self.practice_start_practice_btn, 5) or CommonMethods.wait_for_element_visible(driver, self.video_badge_close_btn, 5) :
                    CommonMethods.elementClick(driver, self.video_badge_close_btn)
                    CommonMethods.elementClick(driver, self.practice_start_practice_btn)
        except:
...
#             window_height = CommonMethods.get_window_height(driver)
    ...
    def verify_chapter_name_color_with_subject_theme(self, driver):
        try:
            css_data = driver.find_element_by_id('com.byjus.thelearningapp.premium:id/video_title_2').value_of_css_property("style")
...
#             self.pause_video(driver)
            ...
            check = CommonMethods.wait_for_element_visible(driver, self.video_play_btn, 60) or CommonMethods.wait_for_element_visible(driver, self.video_frame_pause_btn, 60) or CommonMethods.wait_for_element_visible(driver, self.video_auto_cancelBtn_id, 60)
    ...
    def tap_on_keyFocus_area(self, driver):
        try:
            ...
            if check == True:
                ...
                ele[0].click()
                CommonMethods.wait_for_element_visible(driver, (By.ID,"com.byjus.thelearningapp.premium:id/test_start_button"), 10)
                CommonMethods.elementClick(driver, (By.ID,"com.byjus.thelearningapp.premium:id/test_start_button"))
                CommonMethods.wait_for_element_visible(driver, (By.XPATH,"//android.widget.Button[@text='Submit']"), 10)
    ...
    def verify_video_icon_after_completion(self, driver):
        try:
            ...
            if device == 'mobile':
                ...
                self.verify_true_or_false(driver, check5, 'verify_video_icon_after_completion', "next video chapter name")
