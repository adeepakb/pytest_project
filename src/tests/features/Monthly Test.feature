...
Scenario: Verify when the session ends while a student is taking the assessment


Scenario: Verify the elements in monthly session card when we have post requisite attached


Scenario: Verify that If session type is assessment Session joining pop up should not be showed


Scenario: Verify that if the test is not completed then it will move to completed tab if the session time is over


Scenario: Verify that if user test session expires before the session end time


Scenario: verify that if user not started test and test end time is greater than session end time user is able to take test from completed tab