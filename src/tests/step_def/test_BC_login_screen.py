...
def tap_on_premium_card(login_in):
    ...
    login_in.navigate_to_login_screen()
...
def verify_text(login_in, text):
    details = login_in.verify_login_screen_elements(text=text)
...
def verify_text(login_in):
    details = login_in.verify_login_screen_elements(text='Mobile Number text field')
    check.equal(details.result, True, details.reason)
...
def verify_text(login_in):
    details = login_in.verify_login_screen_elements(text='autofill')
    check.equal(details.result, True, details.reason)


@then('Verify the "Byjus the learning app" app icon.')
def verify_text(login_in):
    details = login_in.verify_login_screen_elements(text='icon')
    check.equal(details.result, True, details.reason)
...
def step_impl(login_in):
    details = login_in.clear_app_data_and_relaunch_the_app()
    check.equal(details.result, True, details.reason)
...
def step_impl(login_in):
    login_in.click_on_country_code_dropdown()


@then('Verify Country Code should be a drop down menu')
def step_impl(login_in):
    details = login_in.is_dropdown_displayed_without_clicking()
    check.equal(details.result, True, details.reason)


@then('By default "India(+91)" should be selected in the drop down menu.')
def step_impl(login_in):
    details = login_in.is_default_country_in_dropdown()
    check.equal(details.result, True, details.reason)
...
def step_impl(login_in):
    details = login_in.verify_country_codes_in_dropdown_in_alphabetical_manner()
    check.equal(details.result, True, details.reason)


@then('Verify numeric keypad gets activated')
def step_impl(login_in):
    details = login_in.is_numeric_pad_displayed()
    check.equal(details.result, True, details.reason)


@when('User taps on Mobile Number field')
def step_impl(login_in):
    login_in.click_on_mobile_number_field()


@then("should accept only Numeric values")
def step_impl(login_in):
    details = login_in.check_numeric_input_in_phone()
    check.equal(details.result, True, details.reason)


@then('Verify error message "Please enter valid mobile number" is displayed below mobile number field')
def step_impl(login_in):
    check.equal(login_in.validate_error_msg(expected_text='Please enter valid mobile number'), True,
                "Error message is not shown or invalid")


@when("Enters  Mobile Number field")
def enter_invalid_phone_number(login_in):
    login_in.enter_phone(phone_num="12345")


@when("User enters valid  having multiple accounts")
@when("User enters valid")
def enter_valid_phone_number(login_in):
    mob = login_in.user_mobile
    login_in.enter_phone(phone_num=mob)


@when("enters  in Mobile Number field")
def enter_invalid_phone_number(login_in):
    login_in.enter_phone(phone_num="1234567890121233")
...
def step_impl(login_in):
    details = login_in.phone_number_field_is_empty()
    check.equal(details.result, True, details.reason)


@given("Device id is saved in server")
def step_impl(login_in):
    details = login_in.login_for_new_user()
    check.equal(details.result, True, details.reason)
    details = login_in.log_out_from_home_page_or_login_page()
    check.equal(details.result, True, details.reason)


@then("Previously logged in user  should be prefilled on Login Screen")
def step_impl(login_in):
    details = login_in.check_mobile_number_field_is_prefilled_with_previous_logged_in_account()
    check.equal(details.result, True, details.reason)


@then("User should navigate to LoginOTPVerificationScreen")
def step_impl(login_in):
    details = login_in.is_otp_screen_shown()
    check.equal(details.result, True, details.reason)
...
def step_impl(login_in):
    details = login_in.is_app_on_screen()
...
def step_impl(login_in):
    login_in.close_dropdown()


@then('login screen should be displayed.')
def step_impl(login_in):
    login_in.verify_login_screen()
...
def step_impl(login_in):
    details = login_in.verify_dialer()
    check.equal(details.result, True, details.reason)
    details = login_in.verify_dialer_with_number(number="+91 9241333666")
    check.equal(details.result, True, details.reason)
...
def step_impl(login_in):
    details = login_in.click_on_continue_button()
    check.equal(details.result, True, details.reason)
    details = login_in.verify_sibling_screen()
    check.equal(details.result, True, details.reason)
