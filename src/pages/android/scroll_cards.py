...
class ScrollCards:
    ...
    def scroll_by_card(self, start_element, end_element):
        ...
        end_x = self.element_location_and_size(end_element, dimension='width') // 2
        end_y = self.element_location_and_size(end_element, ordinates='y')
        action.press(x=start_x, y=start_y).wait(4000).move_to(x=end_x, y=end_y).release().perform()
    ...
    def scroll_by_element(self, start_element, end_element, direction='up'):
        ...
        start_y = self.element_location_and_size(start_element, ordinates='y')
        end_x = self.element_location_and_size(end_element, ordinates='x')
        if direction == 'down':
            end_y = self.element_location_and_size(end_element, ordinates='y') +\
